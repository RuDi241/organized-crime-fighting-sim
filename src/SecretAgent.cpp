...
SecretAgent::SecretAgent(int ID, int rank, int trust, int policeMsqID,
                         int gangID)
    : GangMember(ID, rank, trust, gangID), policeMsqID(policeMsqID) {}

SecretAgent::SecretAgent(const GangMemberMessage &msg, int policeMsqID,
                         int gangID)
    : GangMember(msg, gangID), policeMsqID(policeMsqID) {}
...
void SecretAgent::sendToPolice(InformationMessage message) {
  ...
  if (msgsnd(policeMsqID, &agentMessage, sizeof(AgentMessage) - sizeof(long),
             0) == -1) {
